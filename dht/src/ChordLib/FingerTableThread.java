...
public class FingerTableThread implements Runnable {
	...
	public void run() {
		...
		try {
			System.out.println("Trying to run fingerTableThread on "
					+ node.getNodeKey().toString());
		...
		/*
		 * // next stores the index of the finger to fix n.fix_fingers() next =
		 * next + 1; if (next > m) next = 1; finger[next] =
		 * find_successor(n+2^{next-1});
		 */
		BigInteger two = BigInteger.valueOf(new Integer(2));

		Integer next_plus_one_power_of_two = (int) Math.pow(2,
				fingerToCheck - 1);// oti leei i 8ewria
		try {
			Number160Bit t = node.getNodeKey().getKeyValue()
					.add(BigInteger.valueOf(next_plus_one_power_of_two));
			NodeKey arg_of_findsuccessor = new NodeKey(t, node.getNodeKey()
					.getPID(), node.getNodeKey().getIP());
		...
		if (finger != null) {
			...
			try {
				node.setFinger(fingerToCheck - 1, finger);
			} catch (RemoteException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
