...
public class ChordClient {
	...
	public static void main(String[]args) throws IOException {
		...
		hostName=NodeProcessID.substring(NodeProcessID.indexOf("@")+1, NodeProcessID.length());
		System.out.println("Process ID "+processID+" running on "+hostName);
		...
		if(luckyPeer==null) {
			...
			try {
				System.out.println("bind name for bootstrap "+thisSignature.getRmiRecord());
				Naming.bind(thisSignature.getRmiRecord(), thisNode);
				//Naming.rebind("//:5099/count", v);
				System.out.println("Node's service binded.. this is a bootstrap node");
			...
			sigResThread.start();
			System.out.println("Signature response thread started for bootstrap..");
...
/*to cremove rmi:!!*/String nameToLookup="rmi://"+luckyPeer.getIp().getHostAddress()+":1099/"+luckyPeer.getRmiRecord();
					//firstPeer=(AppNodeInt)Naming.lookup(("//"+luckyPeer.getIp().getHostAddress()+":1099/"+luckyPeer.getRmiRecord()));
					System.out.println("name to look up for NOT BOOTSTRAP"+ nameToLookup);
					firstPeer=(AppNodeInt) Naming.lookup(nameToLookup);
					System.out.println("found a node with process ID" +firstPeer.getNodeKey().getPID()+"running on unknown host on IP: "+firstPeer.getNodeKey().getIP());
			...
			//na ton valoume ston daktulio
				try {
					System.out.println("reached this point now must print from "+ firstPeer.getNodeKey());
