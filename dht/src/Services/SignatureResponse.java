...
public class SignatureResponse implements Runnable {
	...
	private AppNodeSignature signatureToResponse;
	private DatagramPacket signaturePacket;
	...
	public void run() {
		...
		try {
			...
			System.out
					.println("!!Something went wrong in signature response setting!!");
		...
		while (!Thread.interrupted()) {
			...
			try {
				...
				System.out.println("A new request came from "
						+ reqPacket.getAddress().toString());
				buffer = getBytes(signatureToResponse);
				// System.out.println("Answer send is: "+Arrays.toString(buffer));
				signaturePacket = new DatagramPacket(buffer, buffer.length,
						reqPacket.getAddress(), 6000);
				responseSocket.send(signaturePacket);
			} catch (Exception e) {
				System.out
						.println("!!-------Something went wrong in signature response thread-------!!");
