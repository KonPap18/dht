...
public class Multicaster {
	...
	public static AppNodeSignature findTheLuckyOne() throws IOException {
		...
		// Log.addMessage("Sending multicast from client for bootstraping",
		// Log.INFORMATION);
		...
		try {
			...
			AppNodeSignature LuckySignature = (AppNodeSignature) toObject(answer
					.getData());

			listeningSocket.close();
			querySocket.close();
			if (!(LuckySignature == null)) {
				System.out.println("Multicaster says: "
						+ LuckySignature.getProcessID() + " answered");
			}
	...
	private static AppNodeSignature toAppNodeSignature(byte[] bytes)
